...
gd <- gadget_directory("Anchovy2020_3")
...
#ERROR
#Numb20181=(catches20181cakg[1,1]+catches20181alton[1,1]*1000)*(CatchesAllFleetsPTSP$ALL[CatchesAllFleetsPTSP$YEAR==2017 & CatchesAllFleetsPTSP$SEASON==1])/((BIOMTOT$TONPT+BIOMTOT$TONSP)[BIOMTOT$YEAR==2017 & BIOMTOT$SEASON==1])
 Numb20201=(catches20201cakg[1,1]+catches20201alton*1000)*(CatchesAllFleetsPTSP$ALL[CatchesAllFleetsPTSP$YEAR==2019 & CatchesAllFleetsPTSP$SEASON==1])/((BIOMTOT$TONPT+BIOMTOT$TONSP)[BIOMTOT$YEAR==2019 & BIOMTOT$SEASON==1]*1000)

#Numb20182=(catches20182cakgpre[1,1])*(CatchesAllFleetsPTSP$NUMBERSP[CatchesAllFleetsPTSP$YEAR==2017 & CatchesAllFleetsPTSP$SEASON==2])/((BIOMTOT$TONSP)[BIOMTOT$YEAR==2017 & BIOMTOT$SEASON==2])
 Numb20202=(catches20202cakgpre[1,1]+catches20202altonpre*1000)*(CatchesAllFleetsPTSP$ALL[CatchesAllFleetsPTSP$YEAR==2019 & CatchesAllFleetsPTSP$SEASON==2])/((BIOMTOT$TONPT+BIOMTOT$TONSP)[BIOMTOT$YEAR==2019 & BIOMTOT$SEASON==2]*1000)
BIOMTOT$ALL<-BIOMTOT$TONPT+BIOMTOT$TONSP
 BIOMTOT<-add_row(BIOMTOT, YEAR=2020, SEASON=1:2, TONPT=c(catches20201alton,catches20202altonpre),TONSP=c(catches20201cakg[1,1]*0.001,catches20202cakgpre[1,1]*0.001),ALL=c(catches20201alton+catches20201cakg[1,1]*0.001,catches20202altonpre+catches20202cakgpre[1,1]*0.001)) #sacado de Ane279a_9aS_Prov Landings Jan-June 2019.xls porque para Portugal las capturas son cero
 
 BIOMTOTnewyear<-BIOMTOT %>% mutate(newyear=c(1988,1988,rep(1989:2019,each=4))) %>%group_by(newyear)%>%summarise(catonsur=sum(ALL)) 
